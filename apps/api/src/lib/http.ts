...
import { listTraces, writeTrace } from './trace'
...
const download = (name: string, body: Uint8Array, type: string) =>
...
api.get('/api/sponsor-image', (c) => c.redirect('/paymefifty.jpg', 302))
...
api.get('/api/admin/exports/database', async (c) => {
  await auth(c.req.raw)
  if (!env.databaseFile) return fail('当前数据库不是本地文件，无法导出。', 400)
...
api.get('/api/admin/exports/report-traces', async (c) => {
  await auth(c.req.raw)
  const files = Object.fromEntries(
    (await listTraces()).map((row: { id: number; payload: string; reportId: number }) => [
      `report-${String(row.reportId).padStart(6, '0')}-${row.id}.json`,
      strToU8(row.payload)
    ])
  )
  return download('report-traces.zip', zipSync(files), 'application/zip')
})
